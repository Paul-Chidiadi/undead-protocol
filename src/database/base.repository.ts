...
export abstract class BaseRepository<T extends Document> {
  ...
  //   try {
  //     // Build the base query
  //     let query = this.model.find(filterQuery).skip(skip).limit(limit).lean();

  //     // Apply search if provided
  //     if (options.search) {
  //       console.log(options.conditions, options.search);
  ...
  //     // Select fields to omit if specified
  //     if (options.omitFields && options.omitFields.length) {
  //       const omittedFields = options.omitFields
  //         .map((field) => `-${field}`)
  //         .join(' ');
  //       query = query.select(omittedFields);
